...
# def o3d_knn(pts, num_knn):
#     # 确保 pts 是合法的 (N, 3) ndarray, 没有 nan/inf
...
#     if pts.shape[0] < num_knn + 1:
#         raise ValueError(f"Point cloud too small: {pts.shape[0]} points, but need at least {num_knn+1}")
...
#         [success, i_knn, d_knn] = pcd_tree.search_knn_vector_3d(pcd.points[i], num_knn + 1)
#         if success < num_knn + 1:
#             # 不足以形成邻居，填充或跳过（取决于你任务）
#             continue
#         indices.append(i_knn[1:])   # 去掉自身
#         sq_dists.append(d_knn[1:])
