...
def get_video_pose_li(video_path: str, sample_stride: int = 1):
    """Extract pose from video without retargeting.

    Args:
        video_path (str): video pose path
        sample_stride (int, optional): Defaults to 1. Stride for sampling frames.
    ...
    # Assuming dwprocessor returns a dictionary with 'bodies' key containing the pose data
    output_pose = []
    for frame, detected_pose in zip(frames, detected_poses):
       im = draw_pose(detected_pose, *frame.shape[:2])
       output_pose.append(np.array(im))
