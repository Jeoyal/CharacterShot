...
from inference.dwpose.util import draw_pose
...
def get_video_pose(
    ...
    frames = vr.get_batch(list(range(0, min(len(vr), sample_max_length), sample_stride))).asnumpy()
    ...
    for detected_pose in detected_poses:
        ...
        im = draw_pose(detected_pose, height, width, ref_w=pose_canvas_size)
        output_pose.append(np.array(im))
    return np.stack(output_pose)


def get_image_pose(ref_image, pose_canvas_size=2160):
    """process image pose
    ...
    height, width, _ = ref_image.shape
    ref_pose = dwprocessor(ref_image)
    pose_img = draw_pose(ref_pose, height, width, ref_w=pose_canvas_size)
